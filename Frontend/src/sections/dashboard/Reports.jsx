...
// import { useTheme } from '../../context/ThemeContext'; // Ejemplo: Importa tu hook de tema

const Reports = () => {
  // const { isDarkMode } = useTheme(); // Ejemplo: obtén el estado del tema
  // Simulación mientras no se tiene el hook real. Reemplazar por el hook de arriba.
    ...
    datasets: [
      {
        ...
        borderColor: [isDarkMode ? '#1A362C' : '#FFFFFF'],
        borderWidth: 2,
      },
    ],
  };
  ...
  const pieOptions = {
    responsive: true,
    plugins: {
      legend: { position: 'top', labels: { color: isDarkMode ? '#F9FAFB' : '#333' } },
      title: { display: true, text: 'Distribución de Pagos', color: isDarkMode ? '#F9FAFB' : '#333' },
    },
  };
