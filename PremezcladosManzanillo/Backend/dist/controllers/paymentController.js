...
const createPayment = async (req, res) => {
    ...
    try {
        ...
        await (0, notificationHelper_1.sendNotificationToRoles)(['Administrador', 'Contable'], `Nuevo pago registrado por ${userName} para el presupuesto "${budget.title}" (${newAmountInUSD.toFixed(2)} USD).`);
...
const updatePayment = async (req, res) => {
    ...
    try {
        const existingPayment = await prisma_1.default.payment.findUnique({ where: { id }, include: { budget: true } });
        if (!existingPayment)
            return res.status(404).json({ error: 'Pago no encontrado.' });
        if (existingPayment.status !== 'PENDING')
            return res.status(400).json({ error: 'Solo pagos pendientes.' });
