...
exports.deleteUser = exports.updateUser = exports.updateUserRole = exports.getUsers = void 0;
...
const getUsers = async (req, res) => {
    try {
        ...
        const roleAssignments = {}; // { 'rol_id': ['user_id1', 'user_id2'] }
        await Promise.all(allRoles.map(async (role) => {
            ...
            if (resp.ok) {
                ...
                roleAssignments[role.name] = usersInRole.map((u) => u.user_id);
            }
        }));
        // 4. Mapear los roles de vuelta a cada usuario
        const usersWithRoles = auth0Users.map((u) => {
            ...
            const rolesFromMapping = allRoles
                .filter((role) => roleAssignments[role.name]?.includes(u.user_id))
                .map((role) => role.name);
...
const updateUserRole = async (req, res) => {
    ...
    const adminName = req.dbUser?.name || 'Administrador';
    ...
    try {
        ...
        const roleIdsToDelete = currentRoles.map((r) => r.id);
